...
std::unordered_map<uint32_t, UObject*> sfObjectMap::m_sfIdToUObjectMap;
...
UObject* sfObjectMap::GetUObject(uint32_t id)
{
    if (SceneFusion::Get().Service->Session() != nullptr)
    {
        return GetUObject(SceneFusion::Get().Service->Session()->GetObject(id));
    }
    // We only user m_sfIdToUObjectMap when not in a session
    auto iter = m_sfIdToUObjectMap.find(id);
    return iter == m_sfIdToUObjectMap.end() ? nullptr : iter->second;
}
...
void sfObjectMap::Add(uint32_t id, UObject* uobjPtr)
{
    if (SceneFusion::Get().Service->Session() != nullptr)
    {
        Add(SceneFusion::Get().Service->Session()->GetObject(id), uobjPtr);
        return;
    }
    // We only user m_sfIdToUObjectMap when not in a session
    m_sfIdToUObjectMap[id] = uobjPtr;
}
