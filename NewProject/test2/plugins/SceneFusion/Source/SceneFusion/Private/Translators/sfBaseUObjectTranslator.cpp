...
void sfBaseUObjectTranslator::OnRemoveField(sfDictionaryProperty::SPtr dictPtr, const sfName& name)
{
    ...
    if (dictPtr->GetDepth() != 0)
    {
        ...
        if (!CallPropertyHandler(uobjPtr, dictPtr))
        {
            ...
            IsfMissingObject* missingObjPtr = Cast<IsfMissingObject>(uobjPtr);
            if (missingObjPtr != nullptr)
            {
                sfMissingObjectManager::Get().SerializeProperty(missingObjPtr, dictPtr);
            }
        }
        return;
    }
    ...
    FName fname = FName(UTF8_TO_TCHAR(name->c_str()));
    UnrealProperty* upropPtr = uobjPtr->GetClass()->FindPropertyByName(fname);
    if (upropPtr != nullptr)
    {
    ...
    else
    {
        ...
        if (missingObjPtr != nullptr)
        {
            missingObjPtr->SerializedProperties().Remove(fname);
        }
    }
}
    ...
    SetPropertyCallback setProperty)
{
    ...
    else
    {
        ...
        if (missingObjPtr != nullptr)
        {
            sfMissingObjectManager::Get().SerializeProperty(missingObjPtr, propertyPtr);
        }
    }
}
