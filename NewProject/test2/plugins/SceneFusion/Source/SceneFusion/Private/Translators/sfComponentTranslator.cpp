...
void sfComponentTranslator::Initialize()
{
    ...
    m_onApplyObjectToActorHandle = sfUnrealEventDispatcher::Get().OnApplyObjectToActor.AddRaw(this,
        &sfComponentTranslator::OnApplyObjectToActor);
}
...
void sfComponentTranslator::CleanUp()
{
    ...
    sfUnrealEventDispatcher::Get().OnApplyObjectToActor.Remove(m_onApplyObjectToActorHandle);
...
sfObject::SPtr sfComponentTranslator::CreateObject(UActorComponent* componentPtr)
{
    ...
    if (missingComponentPtr != nullptr)
    {
        ...
        sfMissingObjectManager::Get().DeserializeProperties(missingComponentPtr, propertiesPtr);
    }
    else
    {
    ...
    // If the component belongs to a missing actor, get the real creation method from the missing actor.
    AsfMissingActor* missingActorPtr = Cast<AsfMissingActor>(componentPtr->GetOwner());
    if (missingActorPtr != nullptr)
    {
        EComponentCreationMethod* creationMethodPtr = missingActorPtr->ComponentCreationMethods.Find(componentPtr);
        if (creationMethodPtr != nullptr)
        {
            creationMethod = *creationMethodPtr;
        }
    }
    ...
    // If the component belongs to a missing actor, check if the component should have RF_DefaultSubObject set.
    if (missingActorPtr != nullptr && missingActorPtr->DefaultSubComponents.Contains(componentPtr))
    {
        flags |= RF_DefaultSubObject;
    }
    ...
    for (USceneComponent* childComponentPtr : sceneComponentPtr->GetAttachChildren())
    {
        ...
        else
        {
            ...
            // If we aren't in a session, create an empty sfObject for the child actor
            if (m_sessionPtr == nullptr)
            {
                childPtr = sfObjectMap::GetOrCreateSFObject(actorPtr, sfType::Actor);
            }
            // Actor may be pending kill even when its component isn't because of undo
            else if (m_actorTranslatorPtr->IsSyncable(actorPtr))
            {
...
UActorComponent* sfComponentTranslator::InitializeComponent(AActor* actorPtr, sfObject::SPtr objPtr)
{
    ...
    if (componentPtr == nullptr)
    {
        ...
        if (isMissingClass)
        {
            ...
            sfMissingObjectManager::Get().SerializeProperties(missingComponentPtr, propertiesPtr);
            sfMissingObjectManager::Get().AddStandIn(missingComponentPtr);
        }
    }
    else
    {
    ...
    if (creationMethod == EComponentCreationMethod::Instance || missingActorPtr == nullptr)
    {
        componentPtr->CreationMethod = creationMethod;
    }
    else
    {
        // Components without the instance creation method on missing actors will not save properly, so we store the
        // real creation method in a map and instead assign the instance creation method.
        missingActorPtr->ComponentCreationMethods.Add(componentPtr, creationMethod);
        componentPtr->CreationMethod = EComponentCreationMethod::Instance;
        componentPtr->ClearFlags(RF_DefaultSubObject);
    }

    if (missingActorPtr != nullptr && componentPtr->HasAllFlags(RF_DefaultSubObject))
    {
        // Components on missing actors with RF_DefaultSubObject set won't save properly, so clear the flag and store
        // them in an array so we can tell which ones had the flag cleared.
        missingActorPtr->DefaultSubComponents.Add(componentPtr);
        componentPtr->ClearFlags(RF_DefaultSubObject);
    }
    ...
    if (!isRegistered)
    {
        ...
        {
            // Needed for the component to save properly
            actorPtr->AddInstanceComponent(componentPtr);
        }
...
bool sfComponentTranslator::OnUndoRedo(sfObject::SPtr objPtr, UObject* uobjPtr)
{
    ...
    else if (!componentPtr->IsRenderStateCreated() && !componentPtr->IsTemplate() && componentPtr->IsA<USceneComponent>())
    {
