...
void sfLandscapeTranslator::Initialize()
{
    ...
    actorTranslatorPtr->RegisterActorInitializer<ALandscape>([this](sfObject::SPtr objPtr, AActor* actorPtr)
    {
        ...
        else
        {
            ...
            layerMap.Add(FGuid(), TArray<FLandscapeImportLayerInfo>());
            TMap<FGuid, TArray<uint16>> heightData;
            heightData.Add(FGuid(), data);
...
void sfLandscapeTranslator::RegisterHandlers()
{
    ...
    m_onModifiedHandle = sfUnrealEventDispatcher::Get().OnModify.AddLambda(
        [this](UObject* uobjPtr)
    {
...
void sfLandscapeTranslator::Tick(float deltaTime)
{
    if (!ShouldSyncLandscape())
    {
        if (sfConfig::Get().SyncLandscape && GLevelEditorModeTools().GetActiveMode("EM_Landscape") != nullptr)
        {
...
void sfLandscapeTranslator::ApplyServerWeightmapData(ULandscapeComponent* componentPtr, sfProperty::SPtr propPtr)
{
    ...
    // This is a workaround to fix an Unreal bug where some components don't update their rendered textures
    TArray<UTexture2D*>& textures = componentPtr->GetWeightmapTextures(true);
    for (UTexture2D* texturePtr : textures)
    {
        FLandscapeTextureDataInfo* infoPtr = dataInterface.GetTextureDataInfo(texturePtr);
        FLandscapeTextureDataInfoHack* hackPtr = reinterpret_cast<FLandscapeTextureDataInfoHack*>(infoPtr);
        for (int i = 0; i < infoPtr->NumMips(); i++)
        {
            // The Unreal bug has the height sometimes set to 1 when bFull is true, which results in only 1 row of
            // data being updated. The height should be the same as the width, so we set it to the width.
            if (hackPtr->MipInfo[i].bFull)
            {
                hackPtr->MipInfo[i].MipUpdateRegions[0].Height = hackPtr->MipInfo[i].MipUpdateRegions[0].Width;
            }
        }
    }
#endif
...
bool sfLandscapeTranslator::ShouldSyncLandscape()
{
