...
private:
    ...
    /**
     * Hack to expose private struct FFoliageActor
     */
    struct FFoliageActorHack : public FFoliageImpl
    {
#if WITH_EDITORONLY_DATA
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
    TSet<UClass*> m_modifiedActorTypes;
    ...
    /**
     * Sends foliage instances changes to the server, or reverts to the server state if the actor is locked.
     *
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
    /**
     * Sends foliage instances changes to the server for actor foliage types, or reverts to the server state if the
     * actor is locked.
     *
     * @param   AInstancedFoliageActor* actorPtr to sync actor foliage instances for.
     */
    void SyncActorInstances(AInstancedFoliageActor* actorPtr);
#endif
