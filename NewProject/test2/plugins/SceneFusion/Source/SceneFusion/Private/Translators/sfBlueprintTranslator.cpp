    ...
    m_sessionPtr { nullptr }
{
    ...
    // Graph properties
    propertyManager.AddPropertyToForceSyncList("Blueprint", "FunctionGraphs");
    propertyManager.AddPropertyToForceSyncList("Blueprint", "MacroGraphs");
    propertyManager.AddPropertyToForceSyncList("Blueprint", "UbergraphPages");
    propertyManager.AddPropertyToForceSyncList("Blueprint", "DelegateSignatureGraphs");

    // Variables
    propertyManager.AddPropertyToForceSyncList("Blueprint", "NewVariables");
}
...
{
    if (!ShouldSyncBlueprint())
    {
        return;
    }
    m_sessionPtr = SceneFusion::Service->Session();
    m_tickHandle = SceneFusion::Get().OnTick.AddRaw(this, &sfBlueprintTranslator::Tick);
    m_onPropertyChangeHandle = sfUnrealEventDispatcher::Get().OnPropertyChange.AddRaw(
        this, &sfBlueprintTranslator::OnUObjectPropertyChange);

#if ENGINE_MAJOR_VERSION <= 4 && ENGINE_MINOR_VERSION <= 23
    m_onAssetEditorOpenedHandle = FAssetEditorManager::Get().OnAssetEditorOpened().AddRaw(
        this, &sfBlueprintTranslator::OnAssetEditorOpened);
#else
    m_onAssetEditorOpenedHandle =
        GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OnAssetEditorOpened().AddRaw(
            this, &sfBlueprintTranslator::OnAssetEditorOpened);
#endif
}
...
void sfBlueprintTranslator::CleanUp()
{
    ...
    sfUnrealEventDispatcher::Get().OnPropertyChange.Remove(m_onPropertyChangeHandle);
#if ENGINE_MAJOR_VERSION <= 4 && ENGINE_MINOR_VERSION <= 23
    FAssetEditorManager::Get().OnAssetEditorOpened().Remove(m_onAssetEditorOpenedHandle);
#else
    GEditor->GetEditorSubsystem<UAssetEditorSubsystem>()->OnAssetEditorOpened().Remove(m_onAssetEditorOpenedHandle);
#endif
}
...
{
    if (!ShouldSyncBlueprint() || blueprintPtr == nullptr || sfObjectMap::Contains(blueprintPtr))
    {
        return;
    }
    ...
    // Add the blueprint to the object map before we create properties so that we can find the blueprint's sfObject
    // when we create sfObjects for its inner UObjects.
    ...
    if (blueprintPtr->GeneratedClass != nullptr)
    {
        ...
        if (defaultObjectPtr != nullptr)
        {
            ...
            if (sfConfig::Get().SyncBlueprintGraph)
            {
                //Sync graphs
                TSharedPtr<sfGraphTranslator> graphTranslator
                    = SceneFusion::Get().GetTranslator<sfGraphTranslator>(sfType::UEdGraph);
                if (graphTranslator.IsValid())
                {
                    ...
                    for (UEdGraph* graphPtr : allGraphs)
                    {
                        ...
                        graphTranslator->CreateGraphObject(graphPtr);
                    }
                }
            }
        }
...
void sfBlueprintTranslator::OnCreate(sfObject::SPtr objPtr, int childIndex)
{
    ...
    {
        UBlueprint* blueprintPtr = sfObjectMap::Get<UBlueprint>(objPtr->Parent());
...
void sfBlueprintTranslator::OnCreateBlueprint(sfObject::SPtr objPtr)
{
    ...
    if (blueprintPtr == nullptr)
    {
        sfMissingObjectManager::Get().AddMissingObject(name, objPtr);
    ...
    for (sfObject::SPtr childPtr : objPtr->Children())
    {
        ...
        else if (childPtr->Type() == sfType::TemplateComponent)
        {
        ...
        else if (childPtr->Type() == sfType::UEdGraph)
        {
            ...
            if (graphTranslator.IsValid())
            {
                graphTranslator->OnCreateGraph(childPtr);
            }
        }
    }
...
void sfBlueprintTranslator::OnPropertyChange(sfProperty::SPtr propertyPtr)
{
    sfBaseUObjectTranslator::OnPropertyChange(propertyPtr);
    sfObject::SPtr objPtr = propertyPtr->GetContainerObject();
    UBlueprint* blueprintPtr = sfObjectMap::Get<UBlueprint>(objPtr);
    if (blueprintPtr != nullptr)
    {
        m_structurallyModifiedBlueprints.Add(blueprintPtr);
    }
...
void sfBlueprintTranslator::OnListAdd(sfListProperty::SPtr listPtr, int index, int count)
{
    sfBaseUObjectTranslator::OnListAdd(listPtr, index, count);
    sfObject::SPtr objPtr = listPtr->GetContainerObject();
    UBlueprint* blueprintPtr = sfObjectMap::Get<UBlueprint>(objPtr);
    if (blueprintPtr != nullptr)
    {
        m_structurallyModifiedBlueprints.Add(blueprintPtr);
...
void sfBlueprintTranslator::OnListRemove(sfListProperty::SPtr listPtr, int index, int count)
{
...
void sfBlueprintTranslator::SendChanges()
{
    ...
    for (UBlueprint* blueprintPtr : m_dirtyBlueprints)
    {
        ...
        {
            // Send bluprint property changes
            sfPropertyManager::Get().SendPropertyChanges(blueprintPtr, blueprintObjPtr->Property()->AsDict());

            // Check components parent and property changes
            SendComponentChangesRecursive(blueprintObjPtr, blueprintPtr, scsPtr->GetRootNodes());
...
void sfBlueprintTranslator::SendComponentChangesRecursive(
    ...
    const TArray<USCS_Node*>& nodesToCheck)
{
    ...
    for (USCS_Node* nodePtr : nodesToCheck)
    {
        ...
        SendComponentChangesRecursive(componentObjPtr, blueprintPtr, nodePtr->GetChildNodes());
    }
...
void sfBlueprintTranslator::OnAssetEditorOpened(UObject* asset)
{
    UBlueprint* blueprintPtr = Cast<UBlueprint>(asset);
    if (blueprintPtr == nullptr)
    {
        return;
    }

    // Upload if not synced
    UploadBlueprint(blueprintPtr);
}

bool sfBlueprintTranslator::ShouldSyncBlueprint()
{
