...
{
    m_onDeleteHandle = sfUnrealEventDispatcher::Get().OnRemoveAsset.AddRaw(this, &sfAssetTranslator::OnDeleteAsset);
...
{
    sfUnrealEventDispatcher::Get().OnRemoveAsset.Remove(m_onDeleteHandle);
...
void sfAssetTranslator::OnCreate(sfObject::SPtr objPtr, int childIndex)
{
    ...
    UObject* assetPtr = sfLoader::Get().Load(path, className, nullptr);
    if (assetPtr == nullptr)
    {
        return;
    }
