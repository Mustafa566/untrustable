...
void sfFoliageTranslator::Initialize()
{
    ...
    m_onModifyHandle = sfUnrealEventDispatcher::Get().OnModify.AddLambda([this](UObject* uobjPtr)
    {
        // Actor foliage types don't have a component, so we cannot detect when their instances are modified by
        // checking if the component is modified. Actor foliage types have a class of actor they paint. We detect
        // instance changes by detecting if an actor of the class painted by a foliage type was modified on the same
        // frame that a foliage actor was modified.
        if (m_modifiedFoliageActors.Num() != 0 || sfUndoManager::Get().InUndoRedo())
        {
            AActor* actorPtr = Cast<AActor>(uobjPtr);
            if (actorPtr != nullptr)
            {
                // Add the actor's class to a set so we can check if any actor foliage types paint this class on the
                // next tick.
                m_modifiedActorTypes.Add(actorPtr->GetClass());
            }
        }
    });
#endif
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
    sfUnrealEventDispatcher::Get().OnModify.Remove(m_onModifyHandle);
    m_modifiedActorTypes.Empty();
#endif
}
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
    if (m_modifiedActorTypes.Num() > 0)
    {
        for (AInstancedFoliageActor* actorPtr : m_modifiedFoliageActors)
        {
            SyncActorInstances(actorPtr);
        }
        m_modifiedActorTypes.Empty();
    }
#endif
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
                 || infoPtr->Type == EFoliageImplType::Actor
#endif
                )
            {
...
void sfFoliageTranslator::OnListAdd(sfListProperty::SPtr listPtr, int index, int count)
{
    if (listPtr->GetDepth() == 2)
    {
        ...
        UHierarchicalInstancedStaticMeshComponent* componentPtr = GetComponentFromInfo(infoPtr);
        if (componentPtr != nullptr)
        {
            componentPtr->BuildTreeIfOutdated(true, false);
        }
        m_uiStale = true;
        SceneFusion::RedrawActiveViewport();
    }
...
#if ENGINE_MAJOR_VERSION >= 4 && ENGINE_MINOR_VERSION >= 23
void sfFoliageTranslator::SyncActorInstances(AInstancedFoliageActor* actorPtr)
{
    sfObject::SPtr actorObjPtr = sfObjectMap::GetSFObject(actorPtr);
    if (actorObjPtr == nullptr)
    {
    ...
    sfListProperty::SPtr listPtr = objPtr->Property()->AsList();
    // Iterate the foliage meshes to find actor foliage types that paint actor types that are in the modified actor
    // type set.
    for (auto& iter : GetFoliageInfos(actorPtr))
    {
        if (iter.Value.Get().Type == EFoliageImplType::Actor)
        {
            FFoliageActorHack* foliageActor = static_cast<FFoliageActorHack*>(iter.Value.Get().Implementation.Get());
            if (m_modifiedActorTypes.Contains(foliageActor->ActorClass))
            {
                // Iterate the foliage type properties to find the one that matches the foliage type.
                for (auto listIter = listPtr->begin(); listIter != listPtr->end(); ++listIter)
                {
                    sfDictionaryProperty::SPtr propPtr = (*listIter)->AsDict();
                    if (sfPropertyManager::Get().To<UFoliageType>(propPtr->Get(sfProp::Type)) == iter.Key)
