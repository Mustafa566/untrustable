...
void sfUISessionsPanel::LANArea()
{
    ...
    [
        ...
        [
            SNew(SHyperlink)
            .Text(FText::FromString("Contact us to learn how to enable LAN servers for this project"))
...
FReply sfUISessionsPanel::StartSession()
{
    ...
    // Create the custom dialog window for the session settings.
    TSharedRef<SCustomDialog> customDialog = SNew(SCustomDialog)
        .Title(FText::FromString("Session Settings"))
        .DialogContent(sfSettingsPanel::CreateSessionSettingsArea())
        .Buttons({ SCustomDialog::FButton(FText::FromString("Continue")) });

    // Show the session settings as a modal dialog. Block until an input is received.
