...
bool sfPropertyManager::Copy(sfProperty::SPtr destPtr, sfProperty::SPtr srcPtr)
{
    if (destPtr == nullptr || srcPtr == nullptr || destPtr->Type() != srcPtr->Type() ||
        m_atomicProperties.find(srcPtr) != m_atomicProperties.end())
    {
        return false;
    }
    switch (destPtr->Type())
    {
        ...
        case sfProperty::STRING:
        {
            if (!destPtr->Equals(srcPtr))
            {
                destPtr->AsString()->SetString(srcPtr->AsString()->GetString());
            }
            break;
        }
        ...
        default:
        {
            KS::Log::Warning("Unknown property type: " + std::to_string((int)destPtr->Type()), LOG_CHANNEL);
            break;
        }
    }
    return true;
...
void sfPropertyManager::BroadcastChangeEvents()
{
    ...
    for (TPair<UObject*, TSet<UnrealProperty*>>& pair : m_serverChangedProperties)
    {
        ...
        {
            BroadcastChangeEvent(pair.Key, propPtr);
        }
    }
    m_serverChangedProperties.Empty();
}

void sfPropertyManager::BroadcastChangeEvent(UObject* uobjPtr, UnrealProperty* propPtr)
{
...
void sfPropertyManager::Start()
{
    m_onPropertyChangeHandle = sfUnrealEventDispatcher::Get().OnPropertyChange.AddLambda(
...
bool sfPropertyManager::IsSyncable(UObject* uobjPtr, UnrealProperty* upropPtr)
{
    if (IsPropertyInForceSyncList(upropPtr) ||
        (uobjPtr->IsA<UEdGraphNode>() && !upropPtr->GetName().EndsWith("_DEPRECATED")))
    {
        return true;
    }
...
void sfPropertyManager::OnUPropertyChange(UObject* uobjPtr, FPropertyChangedEvent& ev)
{
    ...
    if (IsSyncable(uobjPtr, ev.MemberProperty))
    {
        ...
        // Editing an inline edit condition for another property triggers an event on the property with the condition
        // instead of the condition property that actually changed. So if the property has an inline edit condition,
        // we add it to the local changed properties set to check for changes.
        if (ev.MemberProperty->HasMetaData("EditCondition"))
        {
            FString conditionName = ev.MemberProperty->GetMetaData("EditCondition");
            UnrealProperty* conditionPropPtr = ev.MemberProperty->GetOwnerClass()->FindPropertyByName(*conditionName);
            if (conditionPropPtr != nullptr && conditionPropPtr->HasMetaData("InlineEditConditionToggle"))
            {
                m_localChangedProperties.FindOrAdd(uobjPtr).Add(conditionPropPtr);
            }
        }
    }
}
...
sfProperty::SPtr sfPropertyManager::GetFText(const sfUPropertyInstance& upropInstance)
{
    ...
    sfProperty::SPtr propPtr = sfPropertyUtil::FromText(*textPtr);
    m_atomicProperties.emplace(propPtr);
    return propPtr;
}
...
bool sfPropertyManager::SetFText(const sfUPropertyInstance& upropInstance, sfProperty::SPtr propPtr)
{
    ...
    if (listPtr->Size() == 1)
    {
        ...
        FString text = sfPropertyUtil::ToString(listPtr->Get(0));
    ...
    {
        // stringIds = table, key
        FName table = FName(*sfPropertyUtil::ToString(listPtr->Get(0)));
        FString key = sfPropertyUtil::ToString(listPtr->Get(1));
    ...
    else if (listPtr->Size() == 3)
    {
        ...
        FString nameSpace = sfPropertyUtil::ToString(listPtr->Get(0));
        FString key = sfPropertyUtil::ToString(listPtr->Get(1));
        FString text = sfPropertyUtil::ToString(listPtr->Get(2));
        bool changed = false;
        ...
        if (iter != m_typeHandlers.end())
        {
            ...
            if (subPropPtr->ArrayDim == 1)
            {
                if (upropInstance.DefaultData() == nullptr ||
                    upropInstance.Data() == upropInstance.DefaultData() ||
...
bool sfPropertyManager::SetObject(const sfUPropertyInstance& upropInstance, sfProperty::SPtr propPtr)
{
    ...
    if (propPtr->Type() == sfProperty::REFERENCE)
    {
        ...
        {
            referencePtr = sfObjectMap::GetUObject(objId);
        }
        else
        {
            objPtr = SceneFusion::Service->Session()->GetObject(objId);
            referencePtr = sfObjectMap::GetUObject(objPtr);
        }
    ...
    UObject* assetPtr = sfLoader::Get().LoadFromCache(path, className, propPtr->GetContainerObject());
    if (assetPtr == nullptr || !assetPtr->IsA(tPtr->PropertyClass))
    {
        if (sfLoader::Get().IsUserIdle())
        {
            assetPtr = sfLoader::Get().Load(path, className, propPtr->GetContainerObject());
        }
        else
        {
...
bool sfPropertyManager::SetSoftObject(const sfUPropertyInstance& upropInstance, sfProperty::SPtr propPtr)
{
    ...
    if (className != "" && !UAssetManager::Get().GetAssetRegistry().GetAssetByObjectPath(*path).IsValid())
    {
        ...
        UObject* standInPtr = sfLoader::Get().Load(path, className, propPtr->GetContainerObject());
...
{
    return sfObjectMap::GetUObject(propPtr->GetObjectId());
}
