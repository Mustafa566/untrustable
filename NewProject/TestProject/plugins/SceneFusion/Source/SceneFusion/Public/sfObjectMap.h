...
public:
    ...
    /**
     * Gets the uobject for an sfObject id, or nullptr if the sfObject id has no uobject.
     *
     * @param   uint32_t id to get uobject for.
     * @return  UObject* uobject for the sfObject id.
     */
    static UObject* GetUObject(uint32_t id);
    ...
    /**
     * Adds a mapping between an sfObject id and a uobject.
     *
     * @param   uint32_t id
     * @param   UObject* uobjPtr
     */
    static void Add(uint32_t id, UObject* uobjPtr);
...
private:
    ...
    static std::unordered_map<uint32_t, UObject*> m_sfIdToUObjectMap;// Only used outside of a session
