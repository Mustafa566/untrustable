...
static const FName ExperimentalFeaturesTabName("SF Settings");
...
void sfUI::Initialize()
{
    ...
    m_onLoaderMissingAssetHandle = sfLoader::Get().OnMissingAsset.AddLambda(
    ...
    m_onLoaderRemoveMissingAssetHandle = sfLoader::Get().OnRemoveMissingAsset.AddLambda(
        [this](const FString& path)
    {
    ...
    m_onMissingAssetHandle = sfMissingObjectManager::Get().OnMissingAsset.AddLambda(
        [this](const FString& path)
    {
        m_missingAssetsPanel.AddMissingPath(path);
        RefreshMissingAssetsWarning();
    ...
    m_onRemoveMissingAssetHandle = sfMissingObjectManager::Get().OnRemoveMissingAsset.AddLambda(
        [this](const FString& path)
    {
        m_missingAssetsPanel.RemoveMissingPath(path);
        RefreshMissingAssetsWarning();
...
void sfUI::Cleanup()
{
    ...
    sfLoader::Get().OnMissingAsset.Remove(m_onLoaderMissingAssetHandle);
    sfLoader::Get().OnRemoveMissingAsset.Remove(m_onLoaderRemoveMissingAssetHandle);
    sfMissingObjectManager::Get().OnMissingAsset.Remove(m_onMissingAssetHandle);
    sfMissingObjectManager::Get().OnRemoveMissingAsset.Remove(m_onRemoveMissingAssetHandle);
    ...
    FGlobalTabmanager::Get()->UnregisterNomadTabSpawner(ExperimentalFeaturesTabName);
}
...
void sfUI::InitializeCommands()
{
    ...
    // Map the 'Feedback' menu entry to the action that opens the user's browser to
    // the Scene Fusion feedback.
    m_UICommandListPtr->MapAction(
        sfUICommands::Get().FeedbackPtr,
        FExecuteAction::CreateLambda([this]()
        {
            FString error = "";
            FString url = "https://www.kinematicsoup.com/scene-fusion/feedback?email=";
            sfConfig& settings = sfConfig::Get();
            if (!settings.Email.IsEmpty())
            {
                url += FGenericPlatformHttp::UrlEncode(settings.Email);
            }
...
void sfUI::RegisterSFTab()
{
    ...
    FTabSpawnerEntry& tabSpawner4 = FGlobalTabmanager::Get()->RegisterNomadTabSpawner(
        ExperimentalFeaturesTabName,
        FOnSpawnTab::CreateRaw(this, &sfUI::OnCreateSettingsTab)
    );
    tabSpawner4.SetDisplayName(FText::FromName(ExperimentalFeaturesTabName));
    tabSpawner4.SetMenuType(ETabSpawnerMenuType::Hidden);
}
...
void sfUI::OnConnectComplete(sfSession::SPtr sessionPtr, const std::string& errorMessage)
{
    ...
    if (sessionPtr != nullptr)
    {
        ...
        for (auto iter = sfMissingObjectManager::Get().Iterate(); iter; ++iter)
        {
            m_missingAssetsPanel.AddMissingPath(iter.Key());
        }
        RefreshMissingAssetsWarning();
    }
...
TSharedRef<SDockTab> sfUI::OnCreateSettingsTab(const FSpawnTabArgs& args)
{
    return sfSettingsPanel::CreateSettingsTab();
}
