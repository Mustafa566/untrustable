...
void sfActorTranslator::Initialize()
{
    ...
    m_onActorAddedHandle = sfUnrealEventDispatcher::Get().OnActorAdd.AddRaw(this, &sfActorTranslator::OnActorAdded);
    m_onActorDeletedHandle = sfUnrealEventDispatcher::Get().OnActorDelete.AddRaw(this,
        &sfActorTranslator::OnActorDeleted);
    m_onActorAttachedHandle = sfUnrealEventDispatcher::Get().OnActorAttach.AddRaw(this,
        &sfActorTranslator::OnAttachDetach);
    m_onActorDetachedHandle = sfUnrealEventDispatcher::Get().OnActorDetach.AddRaw(this,
        &sfActorTranslator::OnAttachDetach);
    m_onFolderChangeHandle = sfUnrealEventDispatcher::Get().OnActorFolderChange.AddRaw(this,
        &sfActorTranslator::OnFolderChange);
    m_onLabelChangeHandle = sfUnrealEventDispatcher::Get().OnActorLabelChange.AddRaw(this,
        &sfActorTranslator::OnLabelChanged);
    ...
    GUObjectArray.AddUObjectDeleteListener((FUObjectArray::FUObjectDeleteListener*)this);
...
void sfActorTranslator::CleanUp()
{
    ...
    sfUnrealEventDispatcher::Get().OnActorAdd.Remove(m_onActorAddedHandle);
    sfUnrealEventDispatcher::Get().OnActorDelete.Remove(m_onActorDeletedHandle);
    sfUnrealEventDispatcher::Get().OnActorAttach.Remove(m_onActorAttachedHandle);
    sfUnrealEventDispatcher::Get().OnActorDetach.Remove(m_onActorDetachedHandle);
    sfUnrealEventDispatcher::Get().OnActorFolderChange.Remove(m_onFolderChangeHandle);
    sfUnrealEventDispatcher::Get().OnActorLabelChange.Remove(m_onLabelChangeHandle);
    sfSelectionManager::Get().OnSelect.Remove(m_onSelectHandle);
    sfSelectionManager::Get().OnDeselect.Remove(m_onDeselectHandle);
    GUObjectArray.RemoveUObjectDeleteListener((FUObjectArray::FUObjectDeleteListener*)this);
...
bool sfActorTranslator::IsSyncable(AActor* actorPtr, bool allowPendingKill)
{
    ...
    return actorPtr != nullptr && // the actor is not null and is valid
        ...
        ((actorPtr->IsEditable() && actorPtr->IsListedInSceneOutliner()) || // the actor is shown in the world outliner,
...
sfObject::SPtr sfActorTranslator::CreateObject(AActor* actorPtr)
{
    ...
    if (missingActorPtr != nullptr)
    {
        ...
        sfMissingObjectManager::Get().DeserializeProperties(missingActorPtr, propertiesPtr);
    }
    else
    {
...
AActor* sfActorTranslator::InitializeActor(sfObject::SPtr objPtr, ULevel* levelPtr)
{
    ...
    if (createActor)
    {
        ...
        if (isClassMissing)
        {
            ...
            sfMissingObjectManager::Get().SerializeProperties(missingActorPtr, propertiesPtr);
            sfMissingObjectManager::Get().AddStandIn(missingActorPtr);
        }
    }
    else
    {
...
void sfActorTranslator::OnSelect(AActor* actorPtr)
{
    ...
    else if (actorPtr->IsEditable() && !actorPtr->IsListedInSceneOutliner())
    {
        for (UActorComponent* componentPtr : actorPtr->GetComponents())
        {
            if (componentPtr->IsA<UChildActorComponent>())
            {
                KS::Log::Warning(TCHAR_TO_UTF8(*(actorPtr->GetName() +
                    " can be synced through foliage editing. " +
                    "Property syncing is disabled as that makes Unreal unstable.")));
                break;
            }
        }
    }
...
void sfActorTranslator::NotifyUObjectDeleted(const class UObjectBase *uobjPtr, int index)
{
    AActor* actorPtr = const_cast<AActor*>(Cast<AActor>(static_cast<const UObject*>(uobjPtr)));
    if (actorPtr != nullptr)
    {
        m_uploadList.Remove(actorPtr);
    }
...
void sfActorTranslator::OnUObjectArrayShutdown()
{
