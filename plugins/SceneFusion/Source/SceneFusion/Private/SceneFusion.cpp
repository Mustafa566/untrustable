...
#include "Translators/sfGraphTranslator.h"
...
void SceneFusion::StartupModule()
{
    ...
    TSharedPtr<sfGraphTranslator> graphTranslator = MakeShareable(new sfGraphTranslator);
    ObjectEventDispatcher->Register(sfType::UEdGraph, graphTranslator);
    TSharedPtr<sfGraphNodeTranslator> graphNodeTranslator = MakeShareable(new sfGraphNodeTranslator);
    ObjectEventDispatcher->Register(sfType::UEdGraphNode, graphNodeTranslator);
...
void SceneFusion::OnConnect()
{
    ...
    sfLoader::Get().Start();
    sfUnrealEventDispatcher::Get().Start();
    sfUnrealEventDispatcher::Get().Disable();// Disable events while SF is initializing the level
    ...
    m_onHotReloadHandle = sfUnrealEventDispatcher::Get().OnHotReload.AddRaw(this, &SceneFusion::OnHotReload);
}
...
void SceneFusion::CleanUp()
{
    ...
    sfUnrealEventDispatcher::Get().OnHotReload.Remove(m_onHotReloadHandle);
    ObjectEventDispatcher->CleanUp();
    sfMissingObjectManager::Get().ClearMissingObjects();
    ...
    sfObjectMap::Clear();
    sfLoader::Get().Stop();
    sfUnrealEventDispatcher::Get().Stop();
...
bool SceneFusion::Tick(float deltaTime)
{
    ...
    {
        // Run initialization that has to wait until the first frame
        sfMissingObjectManager::Get().Initialize();// This will crash if we call it from StartupModule
        sfLoader::Get().Initialize();
    }
    m_frameStartTime = FDateTime().Now().GetTicks();
    if (Service->Session() != nullptr && Service->Session()->IsConnected())
    {
        // Disable Unreal events while SF is changing the level
        sfUnrealEventDispatcher::Get().Disable();
    ...
    if (Service->Session() != nullptr && Service->Session()->IsConnected())
    {
        ...
        // Reenable Unreal events when SF is done changing the level
        sfUnrealEventDispatcher::Get().Enable();
    }
...
{
    return Service->Session() != nullptr &&
        FTimespan(FDateTime().Now().GetTicks() - m_frameStartTime).GetTotalMilliseconds() > 
