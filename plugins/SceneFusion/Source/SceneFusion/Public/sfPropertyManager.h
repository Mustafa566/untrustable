...
{
    friend class sfGraphNodeTranslator;
...
public:
    ...
    T* To(sfProperty::SPtr propPtr, T* currentPtr = nullptr)
    {
        ...
        if (propPtr->Type() == sfProperty::REFERENCE)
        {
            ...
            {
                return Cast<T>(sfObjectMap::GetUObject(objId));
            }
            sfObject::SPtr objPtr = SceneFusion::Service->Session()->GetObject(objId);
            T* tPtr = sfObjectMap::Get<T>(objPtr);
            if (tPtr == nullptr && objPtr != nullptr && objPtr->Property()->Type() == sfProperty::Types::DICTIONARY)
            {
                // The object may not be in the sfObject map yet. Try find the object by name
                UObject* outerPtr = sfObjectMap::GetUObject(objPtr->Parent());
                if (outerPtr != nullptr)
                {
                    sfDictionaryProperty::SPtr propertiesPtr = objPtr->Property()->AsDict();
                    sfProperty::SPtr namePropPtr;
                    if (propertiesPtr->TryGet(sfProp::Name, namePropPtr))
                    {
                        FString name = sfPropertyUtil::ToString(namePropPtr);
                        tPtr = Cast<T>(StaticFindObjectFast(UObject::StaticClass(), outerPtr, FName(*name)));
                    }
                }
            }
            return tPtr;
        ...
        T* assetPtr = Cast<T>(sfLoader::Get().LoadFromCache(path, className, propPtr->GetContainerObject()));
        if (assetPtr == nullptr)
        {
            assetPtr = Cast<T>(sfLoader::Get().Load(path, className, propPtr->GetContainerObject()));
        }
    ...
    /**
     * Calls PostEditChangeProperty for a property on an object.
     *
     * @param   UObject* uobjPtr whose property changed.
     * @param   UnrealProperty* propPtr that changed.
     */
    void BroadcastChangeEvent(UObject* uobjPtr, UnrealProperty* propPtr);
    ...
    /**
     * Starts listening for property changes and syncing properties.
     */
    void Start();

    /**
     * Stops listening for property changes and syncing properties.
     */
    void Stop();
...
private:
    ...
    std::unordered_set<sfProperty::SPtr> m_atomicProperties;// Collection properties that must update atomically.
